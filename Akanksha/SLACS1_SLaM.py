'''
Below script uses SLaM for SLACS 1
...
The following are the inputs from Nightingale:
 ...
 - axis ratio: 0.93
 - orientation: 97.2(NOT for SIE, See Table 2 of Paper)
...
'''
DATASET + MASKING + PLOTTING
'''

workspace_path = str(here())
dataset_path = path.join(workspace_path, "Akanksha","slacs_new","slacs0252+0039")
...
settings_autofit = slam.SettingsAutoFit(
    path_prefix=path.join(workspace_path, "Akanksha","SLACS1_output"), #Change this =
    unique_tag='SLACS1_SLaM',
    info=None,
    number_of_cores=1,
    session=None,
)
